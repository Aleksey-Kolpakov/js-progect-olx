...
const addItemToFavouriteCallback = itemId => event => {
  ...
  addItemToFavourite(itemId).then(() => {
    if (event.target.tagName === 'use') {
      ...
      tagUse.setAttribute(
        'href',
        './images/sprite/sprite.svg#icon-favorite-heart',
      );
      tagUse.parentNode.style['opacity'] = '1';
      tagUse.parentNode.addEventListener(
        'click',
        deleteItemFromFavouriteCallback(itemId),
        { once: true },
      );
      return;
    }
...
const deleteItemFromFavouriteCallback = (itemId, inMyCabinet) => event => {
  ...
  deleteItemFromFavourite(itemId).then(() => {
    ...
    if (event.target.tagName === 'use') {
      ...
      tagUse.setAttribute(
        'href',
        './images/sprite/sprite.svg#icon-heart-add-favorite',
      );
      tagUse.parentNode.addEventListener(
        'click',
        addItemToFavouriteCallback(itemId),
        { once: true },
      );
      return;
    }
    if (event.target.tagName === 'svg') {
      ...
      tagSVG.addEventListener('click', addItemToFavouriteCallback(itemId), {
        once: true,
      });
      return;
    }
...
function addOnClickHeartsFetchAddItemToFavourites(favourArray) {
  ...
  itemsArray.forEach(item => {
    ...
    favItemBtnHeart.addEventListener(
      'click',
      addItemToFavouriteCallback(item.dataset.id),
      { once: true },
    );
  });
}
...
export function colorInOrangeHeartsOfFavourites(inMyCabinet = false) {
  getUsersFavouritesByToken().then(favArray => {
    ...
    favArray.map(item => {
      ...
      favItemBtnHeartUseTag.setAttribute(
        'href',
        './images/sprite/sprite.svg#icon-favorite-heart',
      );
      favItemBtnHeart?.addEventListener(
        'click',
        deleteItemFromFavouriteCallback(item._id, inMyCabinet),
        { once: true },
      );
    });
